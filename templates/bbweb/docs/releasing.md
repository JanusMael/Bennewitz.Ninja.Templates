...
A release is one self-contained, single-file server per platform, beside its static assets,
attached to a GitHub Release. Nothing goes to nuget.org, and no credential needs setting up: the
workflow's own token creates the release. The container image CI builds is pushed nowhere; pushing
it to a registry, and deploying it, is a workflow this repository adds when it has somewhere to go.
...
1. `main` is green, including CI's `container` job.
...
`Test`; then, for each of the six platforms, `Publish` and `Archive`; then `Create GitHub Release`.
A failure at any step stops it before the release exists.

## Checking what shipped

On the release page, download one archive, unpack it, run `SiteStem`, and request `/healthz` and

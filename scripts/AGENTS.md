...
|---|---|---|
| `verify-release.cs` | The release gate: pack, install from the `.nupkg` into a template hive of its own, then generate every template, `bbweb` in both variants, and for each assert the tree, build, test and check the conventions offline; `bbpkg`'s is also packed and guarded, and each app is published for this machine, trimmed against its baseline or run and asked for `/healthz` and `/version`. `--published <version>` installs from nuget.org instead. It never touches the global template registration | CI's `verify` job runs it on every push |

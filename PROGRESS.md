...
| 9 · FileServer, AutoVersioning, then DiffView by pull request | next | |
| 10 | not started | |

**Found during steps 7 and 8, not fixed here** (each belongs to its repository's own work):
- XamlQuality: `PROGRESS.md`'s Status row says `main` carries nothing unreleased while its table lists two commits; `docs/avalonia-gotchas.md` lacks the rule-pointer lines for XQ1001 and XQ1002 that its header promises; the backlog names an `XQ1005` although ids are assigned only when a rule is written; `FixturePaths.RepoRoot` and `CompatMapping` comments are stale.
- AssemblyQuality: the root `AGENTS.md` Layout lists `AssemblyRuleResult` as its own file (it is in `IAssemblyRule.cs`); no test covers the `Refuse to release without NUGET_USER` step.
- AppServices and ScopedEditors: listed under Next in their own `PROGRESS.md`. ScopedEditors' README line "`.AvaloniaUI` depends on Semi.Avalonia" names the assembly where the sentence means the package `Bennewitz.Ninja.ScopedEditors.Avalonia`; left for the maintainer to decide.
